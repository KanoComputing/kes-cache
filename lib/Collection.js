...
const Collection = function Collection(cacheName, options) {
...
Collection.prototype.add = async function add(doc) {
    let nd;
    try {
...
Collection.prototype.addToSet = async function addToSet(query, valuesToAdd) {
    const q = {};
    let vals = {};
...
Collection.prototype.get = async function get(query) {
    let result;
    const q = {};
    const { key: searchBy, value: searchValues } = split(query);
    try {
...
Collection.prototype.pushToArray = async function pushToArray(query, valuesToPush) {
    const q = {};
    let vals = {};
    const opts = { multi: false };
    let result;
...
Collection.prototype.remove = async function remove(query, options) {
    const q = {};
    const opts = options || {};
    let result;
    const { key: searchBy, value: searchValues } = split(query);
...
Collection.prototype.removeFromArray = async function removeFromArray(query, valuesToRemove) {
    const q = {};
    let vals = {};
    const opts = { multi: false };
    let result;
...
Collection.prototype.replace = async function replace(query, valuesToUpdate) {
    const { key: searchBy, value: searchValues } = split(query);
    const q = {};
    const opts = { multi: false };
    let result;
...
Collection.prototype.update = async function update(query, valuesToUpdate) {
    const { key: searchBy, value: searchValues } = split(query);
    const q = {};
    const opts = { multi: false };
    let result;
